...
def extract_fields(mol, scf, scf_corr_en, fci, ccsd):
    # get basis set of first atom
    #basis = mol.basis_on_atom(0)
    # set all atoms to same basis set (we need a unified basis for yaml)
    #mol.set_basis_all_atoms(basis)

    # get the geometry in bohr units
    # TODO: change geometry to appropriate units (angstrom, a.u.)
    ...
    one_elec = np.array(ccsd_wfn.H())
    two_elec = np.array(mints.ao_eri())
